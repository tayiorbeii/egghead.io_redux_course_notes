...
Recall that the only argument to `combineReducers()` is an object with a map to our reducers, so we'll start by writing a function that accepts a parameter we'll call "reducers".
...
We then run `reduce()` on the keys because we want to produce a single value that represents the next state. We do this by iterating over every key and running its associated reducer.
